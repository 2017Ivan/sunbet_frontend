    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
      ...
      <div class="flex flex-wrap gap-2 sm:gap-3 w-full sm:w-auto">
        ...
        <button 
          ...
          class="flex-1 sm:flex-none px-4 sm:px-6 py-2.5 sm:py-3 bg-gradient-to-r from-amber-500 to-amber-600 text-slate-900 rounded-xl font-bold hover:from-yellow-400 hover:to-yellow-500 transition-all shadow-lg shadow-amber-500/20 text-sm sm:text-base"
        >
          <span class="flex items-center justify-center gap-1 sm:gap-2">
            <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
import GenerateMatchesModal from './components/GenerateMatchesModal.vue'
...
async function handleGenerate(options) {
  try {
    const result = await matchStore.generateMatches(options)
    closeGenerateModal()
    await loadFixtures()
    alert(result?.message || `${result?.data?.total || 0} matches generated successfully!`)
  } catch (error) {
    alert(error.response?.data?.message || error.message || 'Failed to generate matches')
  }
}
