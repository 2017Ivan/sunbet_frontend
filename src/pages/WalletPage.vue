    ...
    <div class="max-w-screen-xl mx-auto px-4 py-6">
      <div class="flex gap-6">
        ...
        <div class="flex-1 min-w-0 space-y-5">
              ...
              <div class="relative z-10">
                ...
                <div class="mb-4">
                  <p class="text-4xl font-black text-white tracking-tight">
                    {{ balanceVisible
                      ? 'TZS ' + walletStore.mainBalance.toLocaleString()
                      : 'TZS ••••••' }}
                  </p>
                  <p class="text-sm text-[#606060] mt-1">
                    ≈ USD {{ balanceVisible
                      ? (walletStore.mainBalance / 2580).toFixed(2)
                      : '••••' }}
                  </p>
                </div>
            ...
            <div class="bg-[#161616] border border-[#2A2A2A] rounded-[20px] p-5 flex flex-col justify-between">
              <div>
                ...
                <p class="text-2xl font-black text-[#F59E0B]">
                  TZS {{ walletStore.bonusBalance.toLocaleString() }}
                </p>
          ...
          <div
            v-if="error"
            class="flex items-center gap-3 px-4 py-3 bg-[#EF4444]/10 border border-[#EF4444]/25 rounded-[12px]"
          >
            <svg class="w-4 h-4 text-[#EF4444] flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/>
            </svg>
            <p class="text-sm text-[#EF4444]">{{ error }}</p>
            <button class="ml-auto text-[#EF4444] hover:text-white" @click="walletStore.clearMessages()">✕</button>
          </div>
          ...
          <Transition name="tab" mode="out-in">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  ...
                  >
                    ...
                    <span
                      ...
                      :class="selectedMethod === method.id ? 'text-white' : 'text-[#A0A0A0]'"
                    >
                      {{ method.name }}
                    </span>
                ...
                <div class="space-y-4">
                  ...
                  <div>
                    ...
                    <div class="flex gap-2">
                      <input
                        ...
                        class="flex-1 h-11 px-4 rounded-[10px] bg-[#1E1E1E] border border-[#2A2A2A] focus:border-[#A32D2D] text-sm text-white placeholder:text-[#606060] focus:outline-none transition-all uppercase"
                        :class="promoStatus === 'valid'
                          ? 'border-[#22C55E]'
                          : promoStatus === 'invalid'
                          ? 'border-[#EF4444]'
                          : ''"
                    ...
                    <p v-if="promoStatus === 'valid'" class="text-xs text-[#22C55E] mt-1">
                      ✅ {{ promoDescription || '+TZS 10,000 bonus applied!' }}
                    </p>
                    <p v-if="promoStatus === 'invalid'" class="text-xs text-[#EF4444] mt-1">
                      ❌ Invalid promo code
                    </p>
                  </div>
                  ...
                  <div v-if="depositForm.amount" class="bg-[#0D0D0D] border border-[#2A2A2A] rounded-[12px] p-4 space-y-2">
                    ...
                    <div v-if="promoStatus === 'valid'" class="flex justify-between text-sm">
                      ...
                      <span class="text-[#22C55E] font-semibold">+TZS {{ promoBonus.toLocaleString() }}</span>
                    </div>
                    <div class="flex justify-between text-sm border-t border-[#2A2A2A] pt-2">
                      ...
                      <span class="text-white font-black text-base">
                        TZS {{ (Number(depositForm.amount) + (promoStatus === 'valid' ? promoBonus : 0)).toLocaleString() }}
                      </span>
                    </div>
                  ...
                  <div>
                    ...
                    <div class="relative">
                      ...
                      <button
                        ...
                        @click="withdrawForm.amount = walletStore.mainBalance"
                      >
                        MAX
                      </button>
                    </div>
                    <div class="flex justify-between text-[10px] mt-1">
                      ...
                      <span class="text-[#606060]">
                        Available: <span class="text-white font-semibold">TZS {{ walletStore.mainBalance.toLocaleString() }}</span>
                      </span>
                    </div>
            ...
            <div v-else-if="activeTab === 'transactions'" key="transactions">
              ...
              <div v-if="walletStore.isLoading" class="flex justify-center py-12">
                <svg class="animate-spin w-8 h-8 text-[#A32D2D]" viewBox="0 0 24 24" fill="none">
              ...
              <!-- Transactions list -->
              <div v-else class="bg-[#161616] border border-[#2A2A2A] rounded-[18px] overflow-hidden">
                  ...
                  >
                    ...
                    <div class="text-right flex-shrink-0">
                      <p
                        ...
                        :class="['deposit', 'win', 'bonus'].includes(tx.type)
                          ? 'text-[#22C55E]'
                          : 'text-[#EF4444]'"
                      >
    ...
    <AppModal v-model="showSuccessModal" size="sm">
      <div class="flex flex-col items-center gap-4 py-4">
        ...
        <div class="text-center">
          <h3 class="text-lg font-bold text-white">{{ storeSuccessMessage?.title }}</h3>
          <p class="text-sm text-[#606060] mt-1">{{ storeSuccessMessage?.subtitle }}</p>
        </div>
        <div class="w-full bg-[#0D0D0D] rounded-[12px] p-4 space-y-2">
          <div class="flex justify-between text-sm">
            <span class="text-[#606060]">Amount</span>
            <span class="text-white font-bold">{{ storeSuccessMessage?.amount }}</span>
          </div>
          <div class="flex justify-between text-sm">
            <span class="text-[#606060]">Reference</span>
            <span class="text-white font-mono text-xs">{{ storeSuccessMessage?.ref }}</span>
          </div>
          <div class="flex justify-between text-sm">
            <span class="text-[#606060]">Status</span>
            <span class="text-[#22C55E] font-semibold">✅ {{ storeSuccessMessage?.status }}</span>
          </div>
        </div>
        <AppButton variant="primary" fullWidth @click="closeSuccessModal">
          Done
        </AppButton>
      </div>
    </AppModal>
...
const handleDeposit = async () => {
  ...
  if (!depositForm.value.amount || Number(depositForm.value.amount) < 1000) {
    depositErrors.value.amount = 'Minimum deposit is TZS 1,000'
    return
  }
...
const handleWithdraw = async () => {
  ...
  if (!withdrawForm.value.phone) {
    withdrawErrors.value.phone = 'Phone number is required'
    return
  }
  if (!withdrawForm.value.amount || Number(withdrawForm.value.amount) < 1000) {
    withdrawErrors.value.amount = 'Minimum withdrawal is TZS 1,000'
    return
  }
  if (Number(withdrawForm.value.amount) > walletStore.mainBalance) {
    withdrawErrors.value.amount = 'Insufficient balance'
    return
  }
  if (pin.value.join('').length < 4) {
    withdrawErrors.value.pin = 'Enter your 4-digit PIN'
    return
  }
...
watch(storeSuccessMessage, (val) => {
  if (val?.title) showSuccessModal.value = true
})
