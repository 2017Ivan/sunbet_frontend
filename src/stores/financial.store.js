...
export const useFinancialStore = defineStore('financial', {
  ...
  actions: {
    ...
    /**
     * Withdraw funds via mobile money
     */
    async withdraw({ amount, phoneNumber }) {
      ...
      try {
        const result = await financialService.withdraw({ amount, phoneNumber });
        return { success: true, message: result?.message || 'Withdrawal successful', data: result };
      } catch (err) {
        const msg = err.response?.data?.message || err.message || 'Imeshindwa kutoa pesa. Jaribu tena.';
        this.error = msg;
