...
export const useBookingCodeStore = defineStore('bookingCode', () => {
  // ---- State ----
  const currentCode = ref(null) // { code, expiresAt, selections }
  const isLoading = ref(false)
  const error = ref(null)
  const loadedSelections = ref([])
  const codeStatus = ref(null) // { exists, isActive, isExpired, selectionsCount }
  ...
  async function createBookingCodeFromSlip() {
    ...
    const selections = betStore.slip.map(item => ({
      ...
      selection: item.pick || item.selection,
      odds: parseFloat(item.odds) || 0,
      matchName: item.matchName || item.match,
      market: item.market || item.marketKey
    }))
  ...
  function applyLoadedSelectionsToSlip() {
    ...
    // Clear existing slip and add new selections
    betStore.clearSlip()
    loadedSelections.value.forEach(selection => {
      betStore.addToSlip({
        matchId: selection.matchId,
        matchName: selection.matchName || selection.match,
        pick: selection.selection,
        odds: parseFloat(selection.odds) || 0,
        market: selection.market || '1X2',
        match: selection.matchName || selection.match
      })
    })
